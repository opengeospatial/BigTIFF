...
The BigTIFF standard provides a solution for a TIFF file that requires a file size larger than 4 Gigabytes.

A BigTIFF file (and a TIFF) is defined as a sequence of 8-bit bytes, where the bytes are numbered from 0 to N. In a BigTIFF file, the largest possible file is 2^64^ bytes in length.
...
Each _TIFF field_ has an associated Count. This means that all fields are actually one-dimensional arrays, even though most fields contain only a single value.
